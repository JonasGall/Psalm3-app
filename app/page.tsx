...
export default function Psalm3FullSite() {
  ...
  const fetchVerifiedProjects = async () => {
    ...
    if (data) {
      setProjects(data);
      // Valuation calculation for the TVV counter (sum of valuation_amount column)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('applications').insert([{ 
        ...
        is_verified: false // Always false by default for manual vetting
    }]);
    ...
    <div className="min-h-screen bg-[#05070a] text-white selection:bg-cyan-500/30 font-sans pb-20">
              ...
              p.vetting_tier === 'Verified' ? 'border-cyan-400/30' : 'border-white/5 hover:border-white/20'
               ...
               <div className="absolute top-6 left-6 flex gap-2">
                ...
                <div className="space-y-4 mb-10 text-[10px] uppercase font-bold text-gray-500">
                  ...
                  <div className="flex justify-between border-b border-white/5 pb-2">
                    <span>Target Need</span> <span className="text-cyan-400 italic text-right max-w-[120px]">{p.partnership_need}</span>
                  </div>
                </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-20 border-t border-white/5 text-center">
        <div className="flex justify-center gap-6 mb-8">
           <a href="https://x.com/Psalm3_Protocol" target="_blank" className="p-4 rounded-full bg-white/5 hover:bg-cyan-400 hover:text-black transition-all border border-white/10"><Twitter className="w-6 h-6" /></a>
           <a href="https://t.me/YourAdminHandle" target="_blank" className="p-4 rounded-full bg-white/5 hover:bg-cyan-400 hover:text-black transition-all border border-white/10"><MessageCircle className="w-6 h-6" /></a>
        </div>
          ...
          <div className="bg-[#0D1117] border border-white/10 w-full max-w-xl p-12 rounded-[50px] relative z-10 max-h-[90vh] overflow-y-auto">
            {!showSuccess ? (
              <>
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                  ...
                  <select value={formData.need} onChange={(e) => setFormData({...formData, need: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 font-bold outline-none text-white appearance-none">
                     {['Security Audit', 'Market Maker', 'Lead VC', 'KOL Distribution', 'Launchpad', 'Gaming Stack'].map(n => <option key={n} className="bg-[#0D1117]">{n}</option>)}
                  </select>
              ...
              <div className="text-center py-10 animate-in fade-in zoom-in duration-300">
                ...
                <p className="text-gray-400 font-bold mb-8 uppercase text-xs tracking-widest leading-relaxed px-6">
                  Genesis Phase Initialized. Your application is now in the vetting queue.
                </p>
                ...
                <button 
                  onClick={() => window.open('https://t.me/YourAdminHandle', '_blank')} 
                  className="w-full bg-white text-black font-black py-6 rounded-3xl uppercase text-sm flex items-center justify-center gap-3 hover:bg-cyan-400 transition-all shadow-xl"
                >
                  <MessageCircle className="w-4 h-4" /> Message Lead Analyst
                </button>
              </div>
            )}
          </div>
