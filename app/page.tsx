...
export default function Psalm3FullSite() {
  ...
  const fetchVerifiedProjects = async () => {
    ...
    if (data) {
      // 🛡️ LOGIC: Separate Active Queue from Institutional Archive
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white selection:bg-cyan-500/30 font-sans pb-20">
      ...
      {/* --- INSTITUTIONAL ARCHIVE (SUCCESSFUL PROJECTS) --- */}
            ...
            <div key={p.id} className={`bg-[#0D1117] border p-10 rounded-[40px] relative transition-all group ${
              p.vetting_tier === 'Alliance' ? 'border-cyan-400 shadow-[0_0_50px_rgba(6,182,212,0.1)] scale-[1.03]' : 'border-white/5 hover:border-white/20'
               ...
               <div className="absolute top-6 left-6 flex gap-2">
                  {p.vetting_tier === 'Alliance' && <div className="bg-cyan-400 text-black text-[9px] font-black px-4 py-1.5 rounded-full uppercase italic flex items-center gap-1 shadow-lg shadow-cyan-400/20"><Star className="w-3 h-3 fill-black" /> Alliance Partner</div>}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-[#0D1117] border border-white/10 w-full max-w-xl p-12 rounded-[50px] relative z-10 max-h-[90vh] overflow-y-auto shadow-2xl">
            {!showSuccess ? (
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <textarea required placeholder="Brief Institutional Description" rows={3} value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl p-6 font-bold text-white outline-none focus:border-cyan-400 text-sm transition-all" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <select value={formData.need} onChange={(e) => setFormData({...formData, need: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 font-bold outline-none text-white appearance-none">
                    {['Security Audit', 'Market Maker', 'Lead VC', 'KOL Distribution', 'Launchpads'].map(n => <option key={n} className="bg-[#0D1117]">{n}</option>)}
                  </select>
                </div>
                <button type="submit" disabled={isSubmitting} className="w-full bg-cyan-400 text-black font-black py-6 rounded-3xl uppercase text-sm hover:scale-[1.02] transition-all">
              ...
              <div className="text-center py-10 animate-in fade-in zoom-in duration-300">
                ...
                <button onClick={() => window.open('https://t.me/YourAdminHandle', '_blank')} className="w-full bg-white text-black font-black py-6 rounded-3xl uppercase text-sm hover:bg-cyan-400 transition-all">Message Lead Analyst</button>
              </div>
            )}
          </div>
        </div>
