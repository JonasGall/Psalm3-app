...
export default function Psalm3FullSite() {
  ...
  // 1. Fetch Verified Projects on Load
  useEffect(() => {
  ...
  const fetchVerifiedProjects = async () => {
    const { data, error } = await supabase
      .from('applications')
      .select('*')
      .eq('is_verified', true) // Only show vetted projects
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .insert([{ 
        ...
        is_verified: false // New projects start as unverified
      }]);
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white selection:bg-cyan-500/30 font-sans">
      ...
      <header className="py-24 px-6 text-center max-w-5xl mx-auto">
        <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 px-4 py-1 rounded-full text-[10px] font-bold mb-8 tracking-widest">
          <Activity className="w-3 h-3 animate-pulse" /> NETWORK STATUS: ACTIVE
        </div>
        ...
        <p className="text-gray-400 text-lg md:text-xl mb-12 max-w-2xl mx-auto leading-relaxed">
          The elite alliance layer for Web3. We bridge early-stage builders 
          with the industry's most trusted VCs, auditors, and liquidity partners.
        </p>
        <div className="flex flex-col md:flex-row justify-center gap-4">
          ...
          <button className="glass-panel px-12 py-5 rounded-2xl font-bold text-lg hover:bg-white/5 transition-all">
            Browse Alliances
          </button>
        </div>
      </header>
        ...
        <div className="flex justify-between items-end mb-12">
          <div>
            <h2 className="text-4xl font-black flex items-center gap-3 tracking-tighter uppercase italic">
              <Search className="text-cyan-400 w-8 h-8" /> Verified Deals
            </h2>
            <p className="text-gray-500 mt-2">Authenticated projects currently seeking ecosystem partners.</p>
          </div>
          <div className="hidden md:flex gap-3">
            <button className="bg-white/5 p-3 rounded-xl border border-white/10 hover:border-cyan-400/50"><Filter className="w-5 h-5" /></button>
          </div>
        </div>
          ...
          {projects.length > 0 ? (
            projects.map((p) => (
              <div key={p.id} className="bg-[#0D1117] border border-white/5 p-8 rounded-[32px] hover:border-cyan-400/50 transition-all group relative overflow-hidden">
                ...
                <h3 className="text-2xl font-bold mb-2 group-hover:text-cyan-400 transition-colors">{p.project_name}</h3>
                <p className="text-gray-500 text-sm mb-8 leading-relaxed italic">"Currently establishing strategic infrastructure for the {p.chain} ecosystem."</p>
            ...
            <div className="col-span-1 md:col-span-3 py-32 text-center border border-dashed border-white/10 rounded-[40px] bg-white/[0.01]">
              <Zap className="w-10 h-10 text-gray-700 mx-auto mb-4" />
              <p className="text-gray-600 font-mono text-sm tracking-[0.2em] uppercase">No verified projects found. Apply to be the first.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-[#0D1117] border border-white/10 w-full max-w-xl p-10 rounded-[40px] relative z-10 shadow-2xl">
            ...
            <div className="mb-10">
              <h2 className="text-4xl font-black mb-2 tracking-tighter uppercase italic">Genesis App</h2>
              <p className="text-gray-500 text-sm">Initialize the vetting protocol for your venture.</p>
            </div>
            
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-[0.2em] font-black text-cyan-400 ml-1">Project Name</label>
                <input 
                  required 
                  type="text" 
                  value={formData.name}
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 outline-none focus:border-cyan-400 transition-all font-bold" 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2 text-left">
                  ...
                  <select 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 outline-none appearance-none font-bold cursor-pointer"
                  >
      ...
      <footer className="py-20 border-t border-white/5 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 opacity-40">
           <div className="flex items-center gap-2">
             ...
             <span className="font-black text-sm tracking-widest uppercase italic">Psalm3 Hub</span>
           </div>
           <p className="text-[10px] tracking-[0.4em] uppercase">Genesis Terminal v1.0 • 2026</p>
